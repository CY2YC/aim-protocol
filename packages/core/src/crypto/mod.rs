...
pub mod dilithium {
    ...
    #[derive(Zeroize)]
    #[zeroize(drop)]  // ZeroizeOnDrop is a marker trait, use this attribute instead
    ...
    /// ML-DSA signature
    #[derive(Clone, Debug)]
    pub struct SignatureBytes(Box<Vec<u8>>);

    impl SecretKey {
        /// Generate new signing key with cryptographically secure RNG
        pub fn generate<R: CryptoRngCore>(rng: &mut R) -> Self {
            // ML-DSA requires seed-based generation
            let mut seed = [0u8; 32];
        ...
        /// Note: Uses sign_with_ctx (not sign_with_context)
...
pub mod kyber {
    ...
    /// ML-KEM ciphertext
    #[derive(Clone, Debug)]
    pub struct CiphertextBytes(Box<Vec<u8>>);
    ...
    impl CiphertextBytes {
        ...
        pub fn from_bytes(bytes: &[u8; CIPHERTEXT_LENGTH]) -> Option<Self> {
            if Ciphertext::<MlKem768>::try_from(bytes).is_ok() {
                Some(Self(Box::new(bytes.to_vec())))
            } else {
                None
            }
...
pub mod hybrid {
    ...
    impl HybridKem {
        ...
        pub fn generate<R: CryptoRngCore>(rng: &mut R) -> (HybridPublicKey, HybridSecretKey) {
            ...
            let pk = HybridPublicKey { kyber: kyber_pk, x25519: x25519_pk };
            let sk = HybridSecretKey { kyber: kyber_sk, x25519: x25519_sk };
            (pk, sk)
        }
        ...
        pub fn encapsulate<R: CryptoRngCore>(
            ...
            let ct = HybridCiphertext { kyber: kyber_ct, x25519_ephemeral: eph_pk };
...
mod tests {
    ...
    fn test_kyber_serialization() {
        ...
        // Can't directly compare SecretKey, but can test functionality
...
pub mod dilithium {
    ...
    impl SecretKey {
        ...
        /// Deserialize from bytes
        pub fn from_bytes(bytes: &[u8; SECRET_KEY_LENGTH]) -> Option<Self> {
            SigningKey::<MlDsa65>::try_from(bytes).ok().map(|sk| Self(Box::new(sk)))
        }
    }
    ...
    impl SignatureBytes {
        ...
        /// Deserialize from bytes
        pub fn from_bytes(bytes: &[u8; SIGNATURE_LENGTH]) -> Option<Self> {
            Signature::<MlDsa65>::try_from(bytes).ok().map(|s| Self(Box::new(s)))
        }
    }
}
...
pub mod kyber {
    ...
    impl SecretKey {
        ...
        /// Deserialize from bytes
        pub fn from_bytes(bytes: &[u8; SECRET_KEY_LENGTH]) -> Option<Self> {
            DecapsulationKey::<MlKem768>::try_from(bytes).ok().map(|dk| Self(Box::new(dk)))
        }
    }
    ...
    impl CiphertextBytes {
        ...
        /// Deserialize from bytes
        pub fn from_bytes(bytes: &[u8; CIPHERTEXT_LENGTH]) -> Option<Self> {
            Ciphertext::<MlKem768>::try_from(bytes).ok().map(|ct| Self(Box::new(ct)))
        }
    }
