...
async fn main() -> Result<()> {
    ...
    let mut bpf = Ebpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/debug/aim-xdp"
    ))?;
    
    #[cfg(not(debug_assertions))]
    let mut bpf = Ebpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/release/aim-xdp"
    ))?;
...
fn configure_maps(bpf: &mut Ebpf, args: &Args) -> Result<()> {
    ...
    info!("eBPF maps configured: min_reputation={}, zone_id={}", 
...
async fn collect_stats(bpf: Ebpf) {
    ...
    // Get stats map
    let stats_map: HashMap<&aya::maps::MapData, u8, u64> = match 
        HashMap::try_from(bpf.map("STATS").expect("STATS map not found")) {
        Ok(m) => m,
        Err(e) => {
